...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self.title mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.contentView.mas_top).with.offset(kPaddingSpan);
        ...
        [self.picture mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.title.mas_bottom).with.offset(kPaddingSpan);
            make.left.equalTo(self.contentView.mas_left).with.offset(kPaddingCommon);
...
+ (CGFloat)height {
    ...
    dispatch_once(&onceToken, ^{
        ...
        height += ACTIVITYCELL_PIC_HEIGHT + kPaddingCommon + 3 * kPaddingSpan;
    });
    return height;
}
