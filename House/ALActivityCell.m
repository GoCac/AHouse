...
//        self.picture = [[UIImageView alloc] initWithFrame:CGRectMake(kPaddingCommon, kPaddingCommon, ACTIVITYCELL_PIC_WIDTH, ACTIVITYCELL_PIC_HEIGHT)];
        ...
        [self.title mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.contentView.mas_top).with.offset(kPaddingCommon);
        ...
        [self.time mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.picture.mas_bottom).with.offset(0);
            make.left.equalTo(self.contentView.mas_left).with.offset(kPaddingCommon);
...
+ (CGFloat)height {
    ...
    dispatch_once(&onceToken, ^{
        ...
        height += ACTIVITYCELL_PIC_HEIGHT + 2 * kPaddingCommon;
    });
    return height;
}
